...
import com.ailee.retrofit.exception.ApiException;
import com.ailee.retrofit.exception.ResultInterceptedException;
...
public class AdvancedRetrofitHelper {
    ...
    /**
     * 增加一个 {@link Call} 对象 call 到 {@link #callMap} 中
    ...
    /**
     * 增加一个 {@link Disposable} 对象 disposable 到 {@link #disposableMap} 中
        ...
        call.enqueue(new Callback<T>() {
            ...
            public void onResponse(Call<T> call, Response<T> response) {
                ...
                boolean needRefresh = Boolean.parseBoolean(
                        response.headers().get(SMCacheInterceptor.HEADER_NEED_REFRESH));
                if (needRefresh) {
                    // 需要获取最新数据，再发送一次请求
                        ...
                        if (body != null) {
                            ...
                            if (
                                ...
                                if (SUCCESS.equals(code)) {
                                    ...
                                    if (callback != null) {
                                        callback.onError(call, code, msg);
            ...
            public void onFailure(Call<T> call, Throwable t) {
                if (!call.isCanceled()) {
                    ...
                    if (t instanceof ConnectException) {
                        ...
                        status =
                                ErrorStatus.NETWORK_UNAVAILABLE;
                    }
    ...
    private static <T extends BaseResp> void onCallError(Call<T> call, String status,
                                                         AdvancedRetrofitCallback<T> callback) {
        BaseResp errorResp = Util.createErrorResp(status);
        if (
                Util.isIntercepted(errorResp)) {
            if (callback != null) {
